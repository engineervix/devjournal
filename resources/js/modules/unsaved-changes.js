...
export function initializeUnsavedChangesProtection() {
  ...
  const entryForms = Array.from(formsToProtect).map(textarea => textarea.closest('form')).filter(Boolean);
  ...
  function checkForChanges(targetForm = null) {
    ...
    const contentChanged = window.easyMDEEditorInstance && window.easyMDEEditorInstance.isReady()
      ? (window.easyMDEEditorInstance.getValue().trim() !== (originalFormData.contentMarkdown || '').trim())
      : false;
  ...
  function setupBrowserNavigationProtection() {
    ...
    window.addEventListener('popstate', function(e) {
      if (hasUnsavedChanges && !isSubmitting) {
        const shouldLeave = confirm('You have unsaved changes. Are you sure you want to leave this page?');
        if (!shouldLeave) {
  ...
  function setupInAppNavigationProtection() {
    ...
    document.addEventListener('click', function(e) {
      ...
      if (link.textContent.trim().toLowerCase().includes('cancel') ||
