...
Suggest 2–5 tags based on the content (technologies, topics, concepts). Show them and ask the user to confirm or edit. Wait for their answer.

## Step 5 — Save the entry

Once both type and tags are confirmed:

1. Use the Write tool to save the content to `~/.cache/devlog_entry.md`
2. Run:

```bash
devlog add --type <type> --tags "<tag1>,<tag2>" -q < ~/.cache/devlog_entry.md
```
